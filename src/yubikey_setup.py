...
class CredentialDatabase:
    ...
    def add_user(self, user_id: str, username: str, display_name: str, 
    ...
    def add_credential(self, user_id: str, credential_id: bytes, 
                      public_key: bytes, credential_data: bytes,
...
class YubiKeyManager:
    ...
    def register_yubikey(self, username: str, display_name: str = None,
                         is_admin: bool = False) -> bool:
        ...
        authenticator_selection = AuthenticatorSelectionCriteria(
            ...
            resident_key=ResidentKeyRequirement.REQUIRED if Config.REQUIRE_RESIDENT_KEY else ResidentKeyRequirement.DISCOURAGED
        )
        ...
        try:
            ...
            auth_data = self.server.register_complete(
                {
                    "id": websafe_encode(attestation_object.auth_data.credential_data.credential_id),
            ...
            # Log success
            self.db.log_action(user_id_str, "REGISTER", "SUCCESS", 
                             f"Registered new YubiKey for {username}")
            
            print(f"âœ… YubiKey successfully registered for {username}")
            print(f"User ID: {user_id_str}")
            print(f"Credential ID: {websafe_encode(auth_data.credential_data.credential_id)}")
    ...
    def save_backup_codes(self, username: str, codes: List[str]):
        ...
        backup_file = Path(f"backup_codes_{username}_{datetime.now().strftime('%Y%m%d')}.txt")
...
Examples:
  ...
  %(prog)s list-users
    ...
    register_parser.add_argument('--admin', action='store_true', help='Grant admin privileges')
    
    # Authenticate command
    auth_parser = subparsers.add_parser('authenticate', help='Test YubiKey authentication')
...
â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    if args.command == 'register':
        ...
        if users:
            ...
            print(f"{'Username':<20} {'Display Name':<25} {'Created':<20} {'Last Auth':<20} {'Count':<8} {'Admin':<6}")
            print("-" * 110)
            ...
            for user in users:
                ...
                print(f"{username:<20} {display_name:<25} {created:<20} {last_auth_str:<20} {count:<8} {admin_str:<6}")
        else:
